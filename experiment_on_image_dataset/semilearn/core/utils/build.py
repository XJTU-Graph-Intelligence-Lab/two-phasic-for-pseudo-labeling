...
class ResNet18(nn.Module):
    def __init__(self, pretrained=True, num_classes=100):
        ...
        # 移除原有的全连接层
        self.resnet.fc = nn.Identity()  # 用 nn.Identity 替代原 fc 层
        # 定义新的全连接层
        self.custom_fc = nn.Linear(in_dim, num_classes)

    def forward(self, x):
        # 获取特征
        features = self.resnet(x)  # 通过 ResNet 获取特征
        # 通过自定义全连接层进行分类
        output = self.custom_fc(features)
        return {'feat':features, 'logits': output}  # 返回特征和分类输出
...
def get_data_loader(args,
    ...
    if args.new_sample_strategy:
        if sample_num is None:
            if not args.distributed:
                return DataLoader(dset, batch_size=batch_size, shuffle=False, collate_fn=collact_fn,
                            num_workers=num_workers, drop_last=drop_last, pin_memory=pin_memory)
            else:
                if not tp_record: # new strategy training, shuffle=True
                    sampler = DistributedSampler(dset, num_replicas=num_replicas, rank=rank, shuffle=True)
                else: # record train dynamic & eval use
                    sampler = SequentialDistributedSampler(dset, batch_size, num_replicas=num_replicas, rank=rank)
                return DataLoader(dset, batch_size=batch_size, shuffle=shuffle, collate_fn=collact_fn,
                            num_workers=num_workers, drop_last=drop_last, pin_memory=pin_memory, sampler=sampler)
        else: # train_lb num need upsample!
            ...
            return DataLoader(dset, batch_size=batch_size, shuffle=False, num_workers=num_workers, collate_fn=collact_fn,
                        pin_memory=pin_memory, sampler=data_sampler(dset, num_replicas=num_replicas, rank=rank, num_samples=sample_num),
                        generator=generator, drop_last=drop_last)
    else:
        # old strategy: need long time training
