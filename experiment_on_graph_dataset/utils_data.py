...
def load_data(dataset_name, train_percentage=None, val_percentage=None, device=None):
    ...
    if dataset_name in ['cora', 'citeseer', 'pubmed']:
        if train_percentage is None:
            ...
            val_mask = g.ndata['val_mask'].bool().to(device)
            test_mask = g.ndata['test_mask'].bool().to(device)
            train_mask = th.zeros(num_nodes, dtype=bool).to(device)
            for i in range(num_labels):
                class_index = th.where(th.logical_and(g.ndata['label'] == i, ~(val_mask + test_mask)))[0]
                train_mask[random.sample(class_index.tolist(), k=train_percentage)] = True
    ...
    adj = nx.to_scipy_sparse_matrix(nxg, dtype=np.float64)
