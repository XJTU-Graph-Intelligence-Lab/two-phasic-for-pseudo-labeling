...
def selftraining(prediction, entropy, num_k, num_class, train_mask, test_mask, device):
...
def main(args):
    ...
    for i, seed in enumerate(seed_range):
        ...
        print("------------------- Last Train -------------------")
        th.save(train_mask_running, osp.join(path_root, 'origin_pseudo', 'modis',
                                             f'train_mask_running_{args.train_size}_{args.dataset}_{seed}.pth'))
        th.save(pre_labels, osp.join(path_root, 'origin_pseudo', 'modis',
        ...
        with th.no_grad():
            ...
            test_res.append(test_acc[i].item())
            print(test_acc[i])
    ...
    results_dict['test_res'] = test_res
    results_dict['total_time'] = time.time() - time_now
    with open(osp.join(path_root, 'origin_runs', f'{args.name}_{args.dataset}_{args.train_size}_{args.val_size}_'
