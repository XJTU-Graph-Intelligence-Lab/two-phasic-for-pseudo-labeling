...
def selftraining(prediction, aum, num_k, num_class, train_mask, device):
    ...
    for i in range(num_class):
        ...
        sorted_index = th.argsort(aum[class_index], dim=0, descending=True)
...
def main(args):
    ...
    vars(args)['name'] = args.model + '_AUM_MS_' + str(args.num_layers) + '_Layers'
    ...
    for i, seed in enumerate(seed_range):
        ...
        for stage in range(args.stage):
            ...
            aum = th.zeros(labels.shape[0]).to(device)
            ...
            for epoch in range(args.num_epochs_max):
                ...
                with th.no_grad():
                    ...
                    sorted_pre, _ = th.sort(pre, dim=1, descending=True)
                    max_values = sorted_pre[:, 0]
                    second_max_values = sorted_pre[:, 1]
                    aum += (max_values - second_max_values)
            ...
            aum /= args.num_epochs_max
            train_mask_running = selftraining(prediction, aum, args.num_k//args.stage, num_labels, train_mask_running, device)

        print("------------------- Last Train -------------------")
        th.save(train_mask_running, osp.join(path_root, 'origin_pseudo', 'aum',
                                              f'train_mask_running_{args.train_size}_{args.dataset}_{seed}.pth'))
        th.save(pre_labels, osp.join(path_root, 'origin_pseudo', 'aum',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_k', type=int, default=124)
    ...
    parser.add_argument('--num_epochs_max', type=int, default=300)
