...
from semilearn.datasets.utils import split_ssl_data, get_mini_data
...
def get_cifar(args, alg, name, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    ## 降采样方便debug
    if args.is_mini_data:
        data, targets = get_mini_data(data, targets, num_classes, data_dir)
    print('mini data size:',len(data))
