...
def split_ssl_data(args, data, targets, num_classes,
                   ...
                   lb_index=None, ulb_index=None, include_lb_to_ulb=True, load_exist=True, data_dir=None):
    """
    ...
    lb_idx, ulb_idx = sample_labeled_unlabeled_data(args, data, targets, num_classes, 
                                                    ...
                                                    lb_imbalance_ratio, ulb_imbalance_ratio, load_exist=False, data_dir=data_dir)
...
def get_mini_data(data, targets, num_classes, data_dir):
    torch.manual_seed(1111)
    data_mini_idx = []
    targets_mini_idx = []
    if os.path.exists(os.path.join(data_dir, 'data_mini_idx.bin')) and os.path.exists(os.path.join(data_dir, 'targets_mini_idx.bin')):
        data_mini_idx = torch.load(os.path.join(data_dir, 'data_mini_idx.bin'))
        targets_mini_idx = torch.load(os.path.join(data_dir, 'targets_mini_idx.bin'))
    else:
        for i in range(num_classes):
            idx_pre_class = torch.where(torch.tensor(targets)==i)[0]
            # 生成一个随机排列的索引
            random_indices = torch.randperm(idx_pre_class.size(0))
            # 选择前n个不重复的索引
            selected_indices = idx_pre_class[random_indices[:100]].tolist()
            # 使用选择的索引从Tensor中提取对应的元素
            data_mini_idx += selected_indices
            targets_mini_idx += selected_indices
        torch.save(data_mini_idx, os.path.join(data_dir, 'data_mini_idx.bin'))
        torch.save(targets_mini_idx, os.path.join(data_dir, 'targets_mini_idx.bin'))
    data = data[data_mini_idx]
    targets = torch.tensor(targets)[targets_mini_idx].tolist()
    return data, targets